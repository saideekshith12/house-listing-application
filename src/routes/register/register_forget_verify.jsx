...
export const Route = createFileRoute('/register/register_forget_verify')({
    beforeLoad:()=>{
        const token = localStorage.getItem('forgetpassword')    ;
        if(token !== 'forgetpassword'){
            alert("Dont try to enter this page")
            return redirect ({to :'/register/register_forget_password'})
        }
    },
...
function RouteComponent() {
   ...
   const verifytoken = async(e)=>{
    ...
    const response = new Api(`${import.meta.env.VITE_API_URL}/owner/verify-forget-password` , "POST" , {token})
    ...
    navigate({to :'/register/register_reset_password'})
   }
