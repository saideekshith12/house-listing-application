...
export const Route = createFileRoute('/register/register_reset_password')({
  beforeLoad:()=>{
    const token = localStorage.getItem('password')
    if(token !== 'password'){
      alert('Not Authorized to enter')
      return redirect({to:'/register/register_login'})
    }
...
function RouteComponent() {
  ...
  const resetpassword = async (e)=>{
    ...
    const response = new Api(`${import.meta.env.VITE_API_URL}/owner/reset-password` , "POST" , {email,password})
